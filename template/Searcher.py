...
class Searcher(object):
    ...
    # 获取页面元素，使用显示等待时间，最大等待时长默认为SysConfig.find_element_timeout
    def find_element(self, xpath, seconds=SysConfig.find_element_timeout):
        """
        :param xpath: xpath字符串
        :param seconds: 最大等待时长，整型
        :return: 定位到的web_element
        """
        return WebDriverWait(self.driver, seconds).\
            until(expected_conditions.presence_of_element_located((By.XPATH, xpath)))

    def find_elements(self, xpath, seconds=SysConfig.find_element_timeout):
        """
        :param xpath: xpath字符串
        :param seconds: 最大等待时长，整型
        :return: 定位到的web_element列表，List
        """
        self.find_element(xpath, seconds)
